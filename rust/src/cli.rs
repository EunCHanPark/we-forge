...
// ---------------------------------------------------------------------------
// synonym-coverage — supply-side gaps in the ko↔en dictionary
//
// Scans ~/.we-forge/ecc-index.json, aggregates English token frequency across
// marketplace skill descriptions, filters out tokens we already cover (i.e.
// tokens appearing in KO_EN_SYNONYMS values), and surfaces high-frequency
// remainders. These are candidates for Korean alias additions — a token
// appearing in many skill descriptions but lacking a Korean synonym is a
// vector through which Korean prompts can miss matches.
//
// Symmetric to `synonym-candidates`: that one finds DEMAND-side unknowns
// (Korean tokens users typed that we don't cover); this finds SUPPLY-side
// gaps (English tokens skills use that we don't cover).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// skill-regressions — anchor-based ranking regression detector
//
// Reads learning/skill-suggest-regressions.json (a fixture in the repo) which
// asserts (prompt → expected top-1 skill, min_score). On any failure exits
// non-zero so CI / tick.sh can alert. This catches:
//   - marketplace IDF drift that demotes a known-good match
//   - tokenizer / synonym-map regressions on the Korean path
//   - description-override drift (override silently going stale or wrong)
// ---------------------------------------------------------------------------
