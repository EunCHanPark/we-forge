...
| EP-PARITY-002 | Service-manager re-install on Rust-binary upgrade | [TODO] | 2026-05-12 — root cause: after `we-forgectl` swapped from Python script → Mach-O binary, existing machines' launchd plist still had `ProgramArguments=[/usr/bin/env, python3, we-forgectl, daemon]` → `python3 <binary>` SyntaxError → daemon stayed dead with launchd throttle-retrying. Manual hotfix applied on yukibana's Mac (bootout + `we-forgectl install --enable-telegram` regenerates plist to `[<binary>, daemon]`). PENDING: install.sh / install.ps1 (or a version-stamp check) should detect a stale interpreter-prefixed plist and always regenerate it on upgrade; verify systemd unit (`ExecStart=`) and Windows Task Scheduler action have the same hazard. Follow-up to EP-PARITY-001. |
