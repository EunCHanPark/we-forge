...
> 에이전트 7종: `we-forge`(오케스트레이터 — control flow/verdict/queue/ledger) + `pattern-detector` /
> `skill-synthesizer` / `quality-auditor`(합성 파이프라인) + `memory-manager`(영속 메모리 3-tier) +
> `notifier`(텔레그램) + `monitor-sentinel`(`/skill-report` 텔레메트리 요약). 위 박스에 안 보이는
> `memory-manager`는 we-forge 가 tick 시작 시 `load`, 종료 시 `record`로 호출하고, `notifier`는 종료 시
> PASS/ECC_MATCH 있으면 호출.
...
**동작**:
1. `memory-manager` sub-agent (`load`) 호출 → `pointers.md` 에서 lookups (blocklist/primitive_re/ecc_seen/ecc_recs/tick_counter/hwm) 받기
...
5. 각 candidate 에 대해:
   ...
   - `quality-auditor` sub-agent → redact.sh 셀프테스트 preflight + 7개 rubric 검사 후 판정
6. 판정 결과 메모리에 기록 (다음 tick 에서 활용)
7. queue 엔트리 제거 또는 revise_count += 1

#### 단계 ④ — preflight + 7-point rubric 품질 감사

**Preflight (rubric 전에 항상 실행)**: `redact.sh --self-test` 로 시크릿 필터 자체가 정상인지 확인.
실패(스크립트 없음/깨짐/self-test 케이스 실패) 시 → 그 draft 를 보류(verdict `REVISE`, `reason:redact_preflight_failed`,
`revise_count` 안 올림 — 환경 결함이지 draft 결함이 아니므로), **절대 PASS 안 함**. (rubric 3 이 redact.sh 에
전적으로 의존하던 single point of failure 를 막음.)

`quality-auditor` 가 **모두 통과해야 PASS** 하는 7개 기준:
...
6. **의심 동작 없음 (정규식)** — 본문에 다음 패턴 없어야: 외부 URL(localhost 제외) / `sudo`·`su -` /
   `curl | sh`·`eval`·`base64 -d | sh` / 범위 밖 `rm -rf` / `.env`·`.aws/`·`.ssh/`·`id_rsa` 경로 언급
7. **의미-의도 검사** — rubric 6 을 통과한 draft 의 **본문 전체를 의미로** 6개 위험 카테고리로 분류,
   해당 시 REJECT: ① network execution(원격 코드 fetch 후 실행) ② persistence(재부팅/세션 간 자동 실행
   설치 — cron/launchd/rc/settings.json hook) ③ shell bootstrap(외부 인스톨러 호출 — `curl|sh`, `iwr|iex`)
   ④ credential access(키·토큰·`~/.ssh`·`~/.aws`·`.env`·credential helper 읽기) ⑤ lateral movement(다른
   호스트/계정/컨테이너 조작 — `ssh other-host`, `kubectl exec` 등) ⑥ obfuscated execution(`base64 -d|sh`,
   `eval $(...)`, here-doc → interpreter, `python -c` payload). 정규식이 못 잡는 자연어 우회("부트스트랩
   스크립트를 받아서 쉘에 파이프" == `curl|bash`)를 잡기 위한 것. 입력은 draft 본문 텍스트만(외부 호출 금지),
   애매하면 REJECT 쪽으로 기움.

**fixable rubric(1·2·4·5) 하나라도 실패 → REVISE** (최대 3회), 3번째 실패 → REJECT + `rejected.txt` 영구 블록.
**rubric 3·6·7 실패는 revise_count 무관 즉시 REJECT** (재합성으로 못 고치는 위험). preflight 실패는 위에 적은
대로 비-증가형 REVISE.

#### 신규: Verdict 종류 (2026-04-26 기준)

| Verdict | 의미 | 처리 |
|---------|------|------|
| `PASS` | 감사 통과, skill 등록 | `~/.claude/skills/learned/` 배포 |
| `REVISE` | 재작성 요청 (최대 3회) | revise_count += 1 또는 자동 REJECT |
| `REJECT` | 감사 거부, 영구 차단 | `rejected.txt` 독 마킹 |
| `ECC_MATCH` | ECC 마켓플레이스 스킬과 일치 | 추천만, 합성 안 함 (비용 절감) |
| `DROP` | 쉘 기본 명령어 / 자기 참조 | 정규식 blocklist로 자동 drop |
| `SEQ_CANDIDATE` | 다단계 워크플로우 후보 (shadow) | 1주 관찰 후 감사 통합 검토 |

**ECC_MATCH 필수 필드** (2026-04-26 이후):
- `ecc_skill` — 마켓플레이스 스킬 slug
- `ecc_source` — "marketplace" | "learned" | "instinct" | "evolved"
- `match_score` — 매칭 품질 (0-100)
...
- 사용자 선호 포맷이나 특이 케이스를 `lessons.md` 에 축적 (memory-manager가 기록)
...
### 5-2a. we-forgectl 명령어 (ECC 감사 & 모니터링)

**감사 및 품질 검사:**
```bash
# patterns / ledger / rejected.txt 교차 검증
we-forgectl audit
we-forgectl audit --top 10          # top 10 불일치 항목 보기

# ECC_MATCH 품질 분포 검사 (match_score)
we-forgectl ecc-quality
we-forgectl ecc-quality --threshold 80   # 80 이상만 통과로 판정
```

**ECC 마켓플레이스 활용 기록:**
```bash
# ECC 스킬 사용 히스토그램 보기
we-forgectl ecc-trace --group

# 수동 기록 (옵션 포함)
we-forgectl ecc-log "my-skill" "패턴 감지용으로 사용"
we-forgectl ecc-log "my-skill" "..." \
  --match-method "keyword-overlap" \
  --match-score 85 \
  --decision-latency-ms 245
```
...
│   ├── pattern-detector.md           # 큐 중복 제거 + 클러스터링 (skill-index.jsonl 로 dedup)
│   ├── skill-synthesizer.md          # SKILL.md 초안 작성
│   ├── quality-auditor.md            # PASS/REVISE/REJECT 감사 (redact 셀프테스트 + 의미-의도 rubric 7)
│   ├── notifier.md                   # 텔레그램 전담 (Bash 만)
│   ├── memory-manager.md             # agent-memory 3-tier 전담 (Read/Write 만)
│   └── we-forge.md                   # 메인 세션 오케스트레이터 (control flow / verdict / queue / ledger)
├── commands/
...
├── learning/
│   ├── tick.sh                       # cron/launchd 진입점 (CLAUDE_TICK_TIMEOUT=900)
│   ├── redact.sh                     # 시크릿 필터 (self-test 포함)
│   ├── normalize.py                  # 정규화 + 승격 규칙 (MAX_REVISE_COUNT=3)
│   ├── build_ecc_index.py            # skill-suggest 용 ECC 키워드 인덱스 (485 스킬)
│   ├── build-skill-index.sh          # pattern-detector dedup 인덱스 (4개 소스 → skill-index.jsonl)
│   ├── migrate-memory.sh             # MEMORY.md → 3-tier 1회 분할 (업그레이드 시 install.sh가 실행)
│   ├── sequence_normalize.py         # 다단계 워크플로우 N-gram 감지 (shadow mode)
│   ├── backfill_ecc_match.py         # 과거 ECC_MATCH 레코드 enrichment
...
├── agents/
│   ├── we-forge.md                   # 오케스트레이터 (control flow / verdict / queue / ledger 만 담당)
│   ├── monitor-sentinel.md           # 텔레메트리 요약 (read-only)
│   ├── pattern-detector.md           # 큐 → 캐노니컬 후보 (skill-index.jsonl 로 dedup)
│   ├── skill-synthesizer.md          # SKILL.md 초안 작성
│   ├── quality-auditor.md            # PASS/REVISE/REJECT 게이트 (redact 셀프테스트 + 의미-의도 rubric)
│   ├── notifier.md                   # 텔레그램 전담 (Bash 만)
│   └── memory-manager.md             # agent-memory 3-tier 파일 전담 (Read/Write 만)
├── commands/
│   ├── watch-and-learn.md
│   ├── skill-report.md
│   ├── ask-codex.md
...
│   └── stop-telemetry.sh
├── learning/
│   ├── tick.sh                       # CLAUDE_TICK_TIMEOUT=900
│   ├── redact.sh
│   ├── normalize.py
│   ├── build_ecc_index.py            # skill-suggest 용 ECC 키워드 인덱스
│   ├── build-skill-index.sh          # pattern-detector dedup 인덱스 (4개 소스 → skill-index.jsonl)
│   ├── migrate-memory.sh             # MEMORY.md → 3-tier 1회 분할 (업그레이드 시 install.sh가 실행)
│   ├── sequence_normalize.py
│   ├── backfill_ecc_match.py
│   └── data/
│       ├── events.jsonl              # 수집된 이벤트 (append-only)
│       ├── patterns.jsonl            # 정규화된 패턴 빈도 테이블
│       ├── promotion_queue.jsonl     # 학습 대기 큐
│       ├── ledger.jsonl              # 감사 결정 기록 (ECC_MATCH 트레이스 포함)
│       ├── sequence_candidates.jsonl # 다단계 워크플로우 후보 (shadow mode)
...
├── agent-memory/
│   └── we-forge/
│       ├── hot.md                    # 최근 raw decision 로그 (롤링 ~7일, ≤10KB)
│       ├── lessons.md                # 압축된 교훈 + pre-migration 아카이브 (≤5KB)
│       ├── pointers.md               # 기계 파싱용 JSON: blocklist / primitive_re / ecc_seen / ecc_recs / tick_counter / hwm / dead_skill_candidates
│       ├── skill-index.jsonl         # pattern-detector dedup 코퍼스 (24h 마다 재빌드)
│       └── staging/                  # canonical 경로 막힐 때 skill 초안 임시 저장
└── skills/
    └── learned/
...
> **메모리 3-tier 마이그레이션**: 이전엔 `agent-memory/we-forge/MEMORY.md` 단일 파일이었습니다.
> 업그레이드 설치 시 `learning/migrate-memory.sh`가 자동으로 `hot.md` / `lessons.md` / `pointers.md`로
> 분할하고 원본을 `MEMORY.md.legacy`로 보존합니다. `memory-manager` 에이전트가 이 3개 파일의 유일한 writer입니다.
...
rm ~/.claude/agents/{monitor-sentinel,pattern-detector,skill-synthesizer,quality-auditor,notifier,memory-manager,we-forge}.md
...
- 2026-05-12 — 학습 루프 강화 + 오케스트레이터 분해: notifier·memory-manager 신설, agent-memory 3-tier(hot/lessons/pointers), pattern-detector skill-index, auditor redact 셀프테스트 + 의미-의도 rubric, ecc_recs 권위화 (자세히는 CHANGELOG.md)
- **2026-05-15 v0.5.2** — Korean-aware skill-suggest: Python + Rust 양쪽 tokenizer가 Hangul 음절 추출 + 67-entry `KO_EN_SYNONYMS` 양방향 확장. Korean 프롬프트가 영어 마켓 description과 매칭 가능. token cap 30→40으로 cap-saturation 73→19 (-74%). **description-override layer** 신설 — `learning/skill-description-overrides.json`가 marketplace clone을 건드리지 않고 description 보강(plugin update와 충돌 없음). 첫 entry: canary-watch (upstream PR affaan-m/everything-claude-code#1910).
- **2026-05-15 v0.5.3** — 적극 적응 도구. `we-forgectl synonym-coverage` (공급-측 갭: 마켓 영어 토큰 중 한국어 alias 없는 것 surface) + `we-forgectl synonym-candidates` (수요-측 갭: 실제 한국어 프롬프트에 등장한 unknown 토큰 누적 — `~/.we-forge/synonym-candidates.jsonl`) + `we-forgectl skill-regressions` (10 anchor 회귀 테스트). `tick.sh`가 매 7일마다 regression 자동 점검 → 실패 시 Telegram 알림.
- **2026-05-15 v0.5.4** — 랭킹 갭 fix. `tdd-workflow` (test-driven development write tests first) + `design-system` (한국어 디자인 시스템 컴포넌트) 두 프롬프트가 잘못된 스킬에 매칭되던 문제. 5개 Korean dict entry 추가 (디자인/시스템/스타일/일관성/비주얼) + 2 description override + 2 regression anchor.

### 새 CLI 명령 (v0.5.3+)

```bash
we-forgectl synonym-coverage [--top 30] [--min-skills 3]   # 공급-측 사전 갭
we-forgectl synonym-candidates [--top 20] [--hours N]      # 수요-측 사전 갭
we-forgectl skill-regressions [--verbose]                  # 랭킹 회귀 점검
```

세 도구가 함께 **자기 적응 + 회귀 보호** closed loop을 형성:
- coverage: "마켓에 새 단어, 한국어 alias 필요" (공급)
- candidates: "사용자가 쓴 한국어 단어, 사전에 없음" (수요)
- regressions: "이미 작동하던 매칭이 깨짐" (안전망)
- weekly tick + Telegram: 위 셋을 자동 운영
